<template>
    <div>
        <div class="row">
            <div class="col-md-12">
                <h5 class="d-flex align-items-center"><span class="text-primary">Paramétre</span><i class="bx bx-chevron-right bx-sm"></i> <span  data-toggle="collapse" data-target="#modifierU" style="cursor:pointer">Modification du compte</span></h5>
                <div id="modifierU" class="collapse">
                <form action="" v-on:submit.prevent="submitModifierUtilisateur">
                    <div class="card">
                        <div class="card-body">
                            <div class="row">
...
export default {
  ...
  methods: {
    ...
    submitModifierUtilisateur: function () {
      this.$v.$touch()
      if (this.$v.$pendding || this.$v.$error) {
        return
      }
      axios.patch(`/modifierUtilisateur/${this.idU}`, {
        email: this.email,
        pseudo: this.pseudo
      }).then(response => {
